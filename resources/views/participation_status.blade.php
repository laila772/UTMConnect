...
<div class="container mt-5">
    ...
    @forelse($participations as $participation)
        ...
        <div class="status-card">
            <h5>{{ $participation->program->title }}</h5>
            ...
            @if($participation->status === 'rejected' && $participation->rejection_reason)
                <p><strong>Rejection Reason:</strong> {{ $participation->rejection_reason }}</p>
            @endif

            @if($participation->status === 'accepted' && $participation->program->report)
                <a href="{{ asset($participation->program->report) }}" class="btn btn-primary" target="_blank">Download Report</a>
            @endif
        </div>
        @endif
    @empty
